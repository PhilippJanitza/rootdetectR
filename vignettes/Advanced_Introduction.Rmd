...
Detailed (Advanced) manual for `rootdetectR 0.1.0`
...
The `rootdetectR` package comes with another data set for some more complex analysis. The data set called `root_output_multfac2` consists of root measurements of _A. thaliana_ plants (2 wild type strains and 2 mutants) grown under 3 different conditions on ATS plates: 8 days under 20°C, 8 days under 28°C and a shift experiment with 4 days 20°C and subsequently 4 days under 28°C. The length standard of this experiment is labeled with '20mm' and describes standard measurements of 20mm.
...
In addition it is possible to adjust the column names of the combined grouping variable (standard = Label) and the dependent variable (standard = LengthMM).
...
All ANOVA Analysis can be used adjusted to non Rootdetection data sets by adjusting  `col_grouping1`, `col_grouping2` and `col_value` to define grouping and dependet variables.
...
It is possible to change the color of box, jitter or violin plot by using the argument `plot_colours`. To change the colors you have to add a vector with colors (`ggplot2` colors or hex colors) matching the same length than Factor2 factors. To find out more about using colors in `ggplot2` check: 
...
absolute_plot <- plot_abs(root_norm, plot_significance = T, label_delim = ';', 
...
To add statistics to this plot set `plot_significance = T`. The pairwise ANOVA of interaction effects (interaction_twofacaof) is run internally.
...
## Check a normalized Rootdetection data set

If you want to check if your normalized data set fulfills all criteria for future analysis you can use the function `is_root_norm`. Especially if you have a data set which does not come from the standard rootdetection procedure this function gives the possibility to check your data.
```{r}
# check for normalized rootdetection standard
...
__Caution: If there is only a single treatment condition this type of sub setting does not make any sense and the function will print an error message.__
```{r}
...
onefacaov_fac1(root_norm20, col_grouping1 = 'Factor1', col_grouping2 = NULL, col_value = 'LengthMM')
...
root_norm <- sort_label(root_norm, label_delim = ';', control_fac1 = 'yucOx', control_fac2 = '28')

# check sorting again
levels(root_norm$Label)
```

For more advanced sorting of lables a vector containing all the lines in rigth order must be provided with basic r code.
...
sorting_vector <- c("weitar1_1;20", "weitar1_1;28", "Col_0;20", "Col_0;28", "tir1_1afb2_3;20", "tir1_1afb2_3;28", "yucOx;20", "yucOx;28")
...
Future versions will have some improvements and additional funtcions.
