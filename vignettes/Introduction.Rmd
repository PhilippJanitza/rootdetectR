...
To add statistics to this plot set `plot_significance = T` and add the output from the `interaction_twofacaov`.

```{r}
# plot relative data
interaction_tw_anova <- interaction_twofacaov(root_norm, control = '20', label_delim = ';', draw_out = F)
relative_plot <- plot_rel(root_norm, plot_significance = T, 
                          interaction_twofacaov_output = interaction_tw_anova, 
...
# condcut the interaction two way anova of the root_norm_multfac2 dataset
interaction_tw_anova_multfac2 <- interaction_twofacaov(root_norm_multfac2, control = '20', 
...
relative_plot_multfac2 <- plot_rel(root_norm_multfac2, plot_significance = T,
                                   interaction_twofacaov_output = interaction_tw_anova_multfac2,
                                   type = 'jitter', control = '20', letter_height = 10)
...
## Check a normalized Rootdetection dataset

If you want to check if your normalized dataset fulfils all criteria for future analysis you can use the function `is_root_norm`. Especially if you have a dataset which does not come from the standard rootdetection procedure this function gives the possibilty to check your data.
```{r}
...
onefacaov_fac1(root_norm20, grouping_var1 = 'Factor1', grouping_var2 = NULL, dependend_var = 'LengthMM')
...
## important change
plot_rel function with tukey matrix and not with dataframe
