...
There are some features (arguments) to customize the plots to your needs. For more information on customization of legend elements, axes elements, plotting of sample size, letter elements, coloring etc. just check out the man file by typing `?plot_rel` after loading the package. 
...
```{r}
subset_fac2(need_dataset, control = '20')
```
...
2. Possibility to have other Length standards
3. Function to remove NA and LengthPx = 0 from data.set
4. Function to build subsets
5. Add alternative normality tests to `plot_hist` and `check_norm` functions
6. Add internal testing to the package
7. add options of sorting plots - using forcats package?
