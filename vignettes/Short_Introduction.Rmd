...
```
First you need to check if the data meet the criteria of a standard Rootdetection Output. The function `is_root_output()` helps to check the structure of the input data set.
Then you need to clean your data and transform the measured pixels to millimeter as unit. Therefore length standard measurements are used. In this example we measured one centimeter of an ruler with the same camera (zoom) settings as used for the rest of the plant pictures.

In the next step we can calculate relative data, which is described as the different between the treatments. 
```{r}
